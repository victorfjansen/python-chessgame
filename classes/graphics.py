...
class Graphics:
        ...
        for x in range(8):
            for y in range(8):
                pygame.draw.rect(self.screen, board[x][y].color,
                                 (x * self.square_size, y * self.square_size, self.square_size, self.square_size), 8)
            ...
            for y in range(8):
                if board.matrix[x][y].occupant != None:
                    ...
                    if board.location((x, y)).occupant.king == True:
                        pygame.draw.circle(self.screen, COLORS.GOLD.value, self.pixel_coords((x, y)), int(self.piece_size / 1.7),
                                           self.piece_size >> 2)
    ...
    def highlight_squares(self, squares, origin):
        ...
        for square in squares:
            pygame.draw.rect(self.screen, COLORS.HIGH.value, (
    ...
    def draw_message(self, message):
        ...
        self.text_surface_obj = self.font_obj.render(message, True, COLORS.HIGH.value, COLORS.BLACK.value)
