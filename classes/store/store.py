...
class StoreFactory:
    ...
    def store(self, key: str, file_path: str, value: str) -> None:
        self.get_store()[key] = value
        if self.file_path_exists(file_path):
            with open(file_path, "a") as f:
                f.write(value)

    def get_data_from_store(self, file_path: str, callback_action):
        data_dict = {}
    ...
    def delete_from_store(self, key: str) -> None:
        if self.get_store().get(key):
            self.get_store().pop(key)
        else:
    ...
    def has_data_on_store(self, file_path: str) -> bool:
