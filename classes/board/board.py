...
class Board(BoardModel, BoardContract):
    ...
    def blind_legal_moves(self, pixel):
        ...
        x = pixel[0]
        y = pixel[1]
        if self.get_matrix()[x][y].occupant is not None:

            if (not self.get_matrix()[x][y].occupant.king and
                    self.get_matrix()[x][y].occupant.color == COLORS.BLUE.value):
                blind_legal_moves = [self.position(Directions.NORTHWEST.value, (x, y)),
                                     self.position(Directions.NORTHEAST.value, (x, y))]

            elif (not self.get_matrix()[x][y].occupant.king and
                  self.get_matrix()[x][y].occupant.color == COLORS.RED.value):
    ...
    def remove_piece(self, pixel):
        ...
        x = pixel[0]
        y = pixel[1]
        self.get_matrix()[x][y].occupant = None
    ...
    def move_piece(self, pixel_start, pixel_end):
        ...
        self.get_matrix()[end_x][end_y].occupant = self.get_matrix()[start_x][start_y].occupant
