...
class HardEnemy(EnemyContract):
    def __init__(self):
        print("HARD ENEMY SELECTED")

    def get_better_movement_without_adjacent(self, board, pieces_without_adjacent):
        better_movement = []
        for idx, movement in enumerate(pieces_without_adjacent):
            ...
            if not next_direction_position or (len(better_movement) == 0 and movement == pieces_without_adjacent[-1]):
    ...
    def get_random_piece_with_adjacent(self, board, game) -> (Piece, (int, int), (int, int)):
        ...
        if pieces_with_adjacent:
            ...
            random_piece = random.choice(self.get_better_movement_without_adjacent(board, pieces_without_adjacent))
            return random_piece[2], random_piece[1], random_piece[0]
        else:
